...
def save_checkpoint_func(model, optimizer, scheduler, scaler):
    def save_checkpoint(checkpoint_path, epoch):
        torch.save({'epoch': epoch + 1,
                    'model': model.module.state_dict(),
                    'optimizer': optimizer.state_dict(),
                    'scheduler': scheduler.state_dict(),
                    'scaler': scaler.state_dict()}, f'{checkpoint_path}/model_{epoch + 1:06d}.ckpt')

    return save_checkpoint


def load_checkpoint(checkpoint_path, model, optimizer=None, scheduler=None, scaler=None, device=torch.device('cpu')):
    ...
    return state_dict['epoch'], optim_state, scheduler_state, scaler_state
...
def dict_to_device(data: dict, device, device_name):
    new_dic = {}
    for k, v in data.items():
        if isinstance(v, dict):
            v = dict_to_device(v, device, device_name)
        elif isinstance(v, torch.Tensor):
            v = v.to(device, non_blocking=device_name != 'cpu')
        new_dic[k] = v
