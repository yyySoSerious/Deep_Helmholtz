...
def train(args, logger, checkpoint_dir):
    ...
    save_checkpoint = save_checkpoint_func(helper.model, helper.optimizer, helper.scheduler)

    losses = {}
    for epoch in np.arange(helper.initial_epoch, args.epochs):
        epoch = epoch.item()
        if is_distributed:
            helper.train_sampler.set_epoch(epoch)
            helper.val_sampler.set_epoch(epoch)
...
def train_tune(config, checkpoint_dir=None, args=None, helper_args=None):
    ...
    if checkpoint_dir:
        ...
        helper.initial_epoch, optim_state, scheduler_state = load_checkpoint(
            os.path.join(checkpoint_dir, 'checkpoint'), helper.model)
        ...
        if (epoch + 1) % args.save_freq == 0:
            ...
            with tune.checkpoint_dir(step=epoch) as checkpoint_dir:
                ...
                torch.save({'epoch': epoch + 1,
                            ...
                            'scheduler': helper.scheduler.state_dict()}, path)
